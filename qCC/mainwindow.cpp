...
void MainWindow::connectActions()
{
	...
	connect(m_UI->actionCloudCentroid,				&QAction::triggered, this, &MainWindow::doCloudCentroid);
	connect(m_UI->actionFastSampling,				&QAction::triggered, this, &MainWindow::doFastSampling);
	//"Tools"
...
void MainWindow::doActionInvertNormals()//反转所选实体的法线
{
...
void MainWindow::doActionConvertNormalsToDipDir()//转换点云的法线到两个标量域
{
...
void MainWindow::doActionConvertNormalsToHSV()//将云的法线转换到HSV样子字段
{
	if ( !ccEntityAction::convertNormalsTo(	m_selectedEntities,
...
void MainWindow::doComputeBestFitBB()  //最小包围盒
{
...
void  MainWindow::doFastSampling()//快速均匀采样
{
	bool ok;
	int intervval = QInputDialog::getInt(this, QStringLiteral("参数设置:"), QStringLiteral("采样间隔:"), 10, 1, INT_MAX, 50, &ok);
	if (!ok) 
	{
		return;
	}
	...
	for (ccHObject* entity : getSelectedEntities()) 
	{
		...
		if (entity->isKindOf(CC_TYPES::POINT_CLOUD)) 
		{
			ccCloud = static_cast<ccPointCloud*>(entity);
		}
		if (ccCloud) 
		{
			QElapsedTimer eTimer; // 计时
			eTimer.start();
			CCCoreLib::ReferenceCloud sampledIndex(ccCloud);
			for (int i = 0; i < ccCloud->size(); i += intervval) 
			{
				sampledIndex.addPointIndex(i);
			}
			int warnings = 0;
			ccPointCloud* newColud = ccCloud->partialClone(&sampledIndex, &warnings);
			if (warnings) 
			{
				ccLog::Warning("Not enough memory: colors, normals or scalar fields may be missing!");
			}
			ccConsole::Print("[Fast uniform sample] Timing: %3.2f s.", eTimer.elapsed() / 1.0e3);

			if (newColud)
			{
				newColud->setName(ccCloud->getName() + QStringLiteral("_快速采样"));
				if (ccCloud->getParent()) 
				{
					ccCloud->getParent()->addChild(newColud);
				}
				ccCloud->setEnabled(false);
				addToDB(newColud);
				newColud->setDisplay(ccCloud->getDisplay());
				newColud->prepareDisplayForRefresh();
			}
			else 
			{
	...
	if (erros) 
	{
		ccLog::Error("[Fast uniform sample] Errors occurred during the process! Result may be incomplete!");
	}
	refreshAll();
	updateUI();
}

void MainWindow::doCloudCentroid()//计算点云质心
{
	...
	for (int i = 0; i <ccCloud->size(); i++)
	{
		...
		sum += CCVector3d::fromArray(point->u);
	}
	sum /= static_cast<double>(count);
	CCVector3d center = CCVector3d::fromArray(sum.u);
	ccCloud->setEnabled(true);
	QMessageBox::information(nullptr, QStringLiteral("计算质心"),
		QStringLiteral("质心坐标:\n x = ") + QString::number(center.x) +
...
void MainWindow::doActionComputeNormals()//计算所选实体的法线
{
...
void MainWindow::doActionOrientNormalsMST()//用同样的方法重新定位点云的全部法线（最小生成树）
{
...
void MainWindow::doActionOrientNormalsFM()//用同样的方法重新定位点云的全部法线（快速行进法）
{
...
void MainWindow::clearSelectedEntitiesProperty( ccEntityAction::CLEAR_PROPERTY property )//为选定的实体移除法线
{
...
void MainWindow::enableUIItems(dbTreeSelectionInfo& selInfo)
{
	...
	m_UI->actionCloudCentroid->setEnabled(atLeastOneCloud);
	m_UI->actionFastSampling->setEnabled(atLeastOneCloud);
