...
unsigned ccKMeansDlg::getPickedPoints(std::vector<cc2DLabel*>& pickedPoints)
{
	...
	if (m_orderedLabelsContainer)		//顺序的标签容器是否为空
	{
		//获取所有标签
		...
		{
			ccLog::Error(QStringLiteral("内存不足！"));
			return 0;
		}
...
void ccKMeansDlg::updateList()
{
	...
	tableWidget->setRowCount(count);		//设置对话框的行数
	...
	if ( (count - oldRowCount) > 0 )
	{
		...
		{
			tableWidget->setVerticalHeaderItem( i, new QTableWidgetItem );		//添加一个头选项

			for ( int j = 0; j < 4; ++j )
			{
				tableWidget->setItem( i, j, new QTableWidgetItem );		//添加四个格子
			}
		}
	}
...
void ccKMeansDlg::processPickedPoint(const PickedItem& picked)
{
	...
	newLabel->setVisible(true);			//设置标签可见
